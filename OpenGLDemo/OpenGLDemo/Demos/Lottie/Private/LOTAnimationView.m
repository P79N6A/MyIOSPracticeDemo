...
//#import "QQTlibDyPublicDefault.h"
...
- (void)playFromFrame:(nonnull NSNumber *)fromStartFrame
  ...
  animation.repeatCount = _loopAnimation ? HUGE_VALF : self.repeatTime;
...
#define RDM_EVENT_LOTTIE_ANIMATION                @"lottieAnimation"

- (void)reportLottieTypeToRDM
{
    if(!self.SDPType){
#if TARGET_IPHONE_SIMULATOR
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"error" message:@"lottie 数据上报需要指定业务名称.\n方法：在LOTAnimationView头文件.SDPType属性描述中注册字符串，填入SDKType。注意字符串不要相同" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alertView show];
#endif
    }
    //上报至灯塔
    NSMutableDictionary *paramDict = [NSMutableDictionary dictionary];
    [paramDict setObject:self.SDPType ? self.SDPType : @"-1" forKey:@"animationID"];
    //[QQTlibDyPublicDefault doReport:RDM_EVENT_LOTTIE_ANIMATION  isSucceed:YES elapse:0 size:0 params:paramDict reportImmediately:YES];
}
