...
//
@implementation CameraCuberGLESView
...
- (void)setupProgram
{
    NSString * vertexShaderPath = [[NSBundle mainBundle] pathForResource:@"VertexCuber"
                                                                  ofType:@"glsl"];
    NSString * fragmentShaderPath = [[NSBundle mainBundle] pathForResource:@"FragCuber"
                                                                    ofType:@"glsl"];
...
- (void)setupTextures
{
    ...
    _textureIndex = 1;  // Current bind texture for stage 0.
    _wrapMode = GL_REPEAT;
...
//        _textureIndex = _textureIndex == 1 ? 0:1;
...
//        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, texture.width, texture.height, GL_RGBA, GL_UNSIGNED_BYTE, rgbTexture.RGBA);
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, size.width, size.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, rgbTexture.RGBA);
...
- (void)setImageTexture:(UIImage*)texture
{
    _textures[_textureIndex] = [TextureHelper createTexture:texture];
    //[self updateSurface];
    [self render];
}
...
- (void)render
{
    ...
    glViewport(0, 0, self.frame.size.width*_contentScale, self.frame.size.height*_contentScale);
